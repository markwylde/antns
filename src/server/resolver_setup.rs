...
fn setup_macos_resolver(port: u16) -> Result<()> {
    ...
    println!("All .ant and .autonomi domains will now resolve via localhost:{}", port);
...
fn check_linux_resolver(port: u16) -> Result<bool> {
    ...
    if !std::path::Path::new(&ant_config).exists() || !std::path::Path::new(&autonomi_config).exists() {
        return Ok(false);
    }
...
fn add_nrpt_rule(domain: &str, port: u16) -> Result<()> {
    ...
    let status = Command::new("powershell")
        .args(&[
            "-Command",
            &format!("Add-DnsClientNrptRule -Namespace '{}' -NameServers '{}'", namespace, nameserver)
        ])
        .status()
